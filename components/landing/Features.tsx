  ...
  return (
    <section id="product" className="border-b border-line bg-mist">
      <div className="mx-auto max-w-[1180px] px-6 py-24">
        ...
        {/* Cards respond to being hovered — lift, underglow, border. They do
            NOT track the cursor: at this size a chasing glow reads as noise. */}
        <ul className="mt-12 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map(({ Icon, name, copy, span, feature }) => (
            <li key={name} className={span ?? ""}>
              <div
                className={`card-lift group relative flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-white p-5 ${
                  feature ? "sm:p-6" : ""
                }`}
              >
                {feature && (
                ...
                <span
                  className={`relative grid place-items-center rounded-xl border border-line bg-sky text-cyan-dark transition group-hover:border-cyan/40 ${
                    feature ? "h-11 w-11" : "h-9 w-9"
                  }`}
                >
                  <Icon
                    className={`fx-lift-icon ${feature ? "h-5 w-5" : "h-[17px] w-[17px]"}`}
                  />
                </span>
