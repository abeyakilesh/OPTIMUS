...
import { Globe, MonitorIcon, Check, ArrowRight } from "../landing/Icons";
...
export default function PlatformsV2() {
  return (
    <section className="relative overflow-hidden border-b border-line bg-white">
      <div
        aria-hidden
        className="fx-aurora-a pointer-events-none absolute -left-40 bottom-0 -z-10 h-[440px] w-[440px] rounded-full blur-2xl"
      />
      ...
      <div className="mx-auto max-w-[1180px] px-6 py-24">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-2">
          {PLATFORMS.map(({ Icon, name, tag, points, cta }, i) => (
            <Reveal key={name} variant={i === 0 ? "left" : "right"} delay={i * 120}>
              <Spot>
                <div className="fx-spot fx-edge fx-lift group flex h-full flex-col rounded-2xl border border-line bg-white p-7">
                  <div className="flex items-center gap-3">
                    <span className="grid h-10 w-10 place-items-center rounded-xl border border-line bg-sky text-cyan-dark transition group-hover:border-cyan/45">
                      <Icon className="fx-bounce h-[18px] w-[18px]" />
                    </span>
                    <div>
                  ...
                  <ul className="mt-6 space-y-2.5">
                    {points.map((p) => (
                      <li key={p} className="flex items-start gap-2.5 text-[13.5px] leading-relaxed text-body">
                  ...
                  >
                    <span className="fx-ulink">{cta}</span>
