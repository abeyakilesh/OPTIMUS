import { Reveal, Scramble } from "./fx";
...
/** A rail with dots travelling along it (E31–E33). */
function Rail({ dots = 2, reverse = false }: { dots?: number; reverse?: boolean }) {
  return (
...
export default function KernelV2() {
  return (
      ...
      <div className="mx-auto max-w-[1180px] px-6 py-24">
        ...
        <div className="mt-16 grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,0.85fr)] lg:gap-0">
          ...
          <div className="space-y-3">
            {SOURCES.map(({ Icon, name, note }, i) => (
              <Reveal key={name} variant="left" delay={i * 90}>
                <div className="flex items-center">
                  <div className="group flex w-full max-w-[230px] items-center gap-2.5 rounded-xl border border-line bg-white px-3 py-2.5 transition hover:border-cyan/45 lg:w-auto">
                    <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-sky text-cyan-dark">
                      <Icon className="fx-bounce h-[15px] w-[15px]" />
                    </span>
          ...
          <Reveal variant="scale" delay={200}>
            <div className="fx-beat relative mx-auto w-full max-w-[330px] rounded-2xl bg-white p-5 shadow-[0_30px_70px_-45px_rgba(6,182,212,.7)]">
          ...
          {/* outputs */}
          <div className="space-y-3">
            {OUTPUTS.map(({ Icon, name, note }, i) => (
              <Reveal key={name} variant="right" delay={i * 90 + 240}>
                <div className="flex items-center">
                  <Rail dots={1} reverse />
            ...
            {/* mobile-only vertical connector so the flow still reads stacked */}
        ...
        {/* ══ the shared bus (E33) — every capability rides the same lane ══ */}
